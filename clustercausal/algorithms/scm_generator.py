...
class SCMGenerator:
    ...
    def __init__(
        ...
        num_nodes,
        num_clusters,
        in_cluster_connectivity=0.5,
        between_cluster_connectivity=0.2,
        seed=None,
    ):
        """
        ...
        cluster_probabilities = np.random.uniform(
            low=0.5, high=1, size=num_clusters
        )
        self.cluster_probabilities = cluster_probabilities / np.sum(
        ...
        for node in self.nodes:
            ...
            self.cluster_assignment += [
                np.random.choice(
                    self.num_clusters, p=self.cluster_probabilities
                )
            ]
        ...
        for cluster in range(num_clusters):
            self.cluster_sizes += [
                np.sum(np.array(self.cluster_assignment) == cluster)
            ]
        ...
        for cluster in range(self.num_clusters):
            self.clusters[cluster] = (
                ...
                np.where(np.array(self.cluster_assignment) == cluster),
            )
    ...
    def make_dag(self, seed=None, directed=True):
        ...
        while acyclicity == False:
            self.graph = nx.random_partition_graph(
                ...
                p_in=self.in_cluster_connectivity,
                p_out=self.between_cluster_connectivity,
                seed=self.seed,
                directed=directed,
            )
            for u, v in self.graph.edges:
                if ((u, v) in self.graph.edges()) and (
                    (v, u) in self.graph.edges()
                ):
                    self.graph.remove_edge(v, u)
            for u, v in self.graph.edges:
                # np.random.seed(self.seed)
                self.edge_weights[(u, v)] = np.round(
                    np.random.uniform(low=-2, high=2), decimals=2
                )
    ...
    def scm_equations(self):
        ...
        for node in self.nodes:
            ...
            for parent in self.graph.predecessors(node):
                equations[
                    node
                ] += f"{self.edge_weights[(parent, node)]} * X{parent} + "
        for node in self.nodes:
    ...
    def generate_data(self, samples=1000):
        ...
        for node in self.topological_order:
            ...
            for parent in self.parents[node]:
                data_node += (
                    self.edge_weights[(parent, node)] * data[parent, :]
                )
            data[node, :] = data_node
        self.data = data
